<template>
  <div>
    <div class="type flex a-center">
      <div class="t-item" v-for="(item, index) in typeList" :key="index" :class="{active: activeIndex === index}" @click="clickType(item, index)">
        <div class="t-name">{{item.name}}</div>
      </div>
    </div>
    <div class="content flex a-center" v-if='cat !== "5001"'>
...
export default {
  ...
  methods: {
    getData() {
      api.getSinger(this.cat, this.initial).then(res => {
        if (res.code === 200) {
...
.active {
  background: rgb(198,47,47) !important;
  color: #fff;
}
</style>